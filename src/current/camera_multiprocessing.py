...
#
# Frame buffer:
#   Raw RGB uint8, shape (DISPLAY_H, DISPLAY_W, 3)
#   BGR->RGB conversion happens once in find_objects before writing here.
#
# Result buffer — fixed layout, no pickle:
#   N_TILES slots, each slot:
#     [0]       float32  detection count (cast to int on read)
#     [1..end]  float32  x1, y1, x2, y2, conf, class_idx  per detection
...
FLOATS_PER_DET    = 6    # x1, y1, x2, y2, conf, class_idx
...
def result_buffer_size(n_tiles):
    return n_tiles * SLOT_FLOATS * 4   # bytes
...
class ObjectLocation:
    ...
    depth_m:    float
    pos_camera: np.ndarray     # [x, y, z] camera frame (m)
    pos_world:  np.ndarray     # [x, y, z] world frame  (m), or None
...
def get_tile_meta(frame_w, frame_h, tile_w, tile_h, overlap):
    """Returns [(x_off, y_off), ...] — tile geometry only, no image data."""
...
def is_contained(inner, outer, threshold=0.8):
    """True if >threshold of inner's area overlaps outer."""
...
def detection_worker(
    ...
    result_shm_name,
    result_size,
    tile_ready,      # mp.Event  — main signals a tile is ready to process
    tile_done,       # mp.Event  — worker signals tile result is written
    tile_idx_val,    # mp.Value('i') — tile index to process
    x_off_val,       # mp.Value('i') — tile x offset in frame
    y_off_val,       # mp.Value('i') — tile y offset in frame
    stop_event,      # mp.Event  — signals clean shutdown
):
    """
    Runs in its own process — own GIL, own darknet network handle.
    Reads tile from shared frame memory, writes results to shared result memory.
    No data crosses the process boundary via pickle.
    """
    ...
    # Zero-copy views into shared memory
    ...
    while not stop_event.is_set():
        ...
        # Copy tile out of shared memory before main process can overwrite it
...
class ObjectDetector:
    """
    Manages WORKER_COUNT worker processes.
    Each frame:
      1. BGR→RGB conversion happens once, written to shared frame memory.
      2. Tiles dispatched round-robin to workers via Events + Values.
      3. Main process collects tile_done signals.
      4. Results read from shared result memory and NMS'd.
    """
    def __init__(self):
        # Tile geometry — computed once, shared as plain list (no image data)
        self.tiles_meta = get_tile_meta(
            DISPLAY_W, DISPLAY_H, 416, 416, OVERLAP
        )
    ...
    def find_objects(self, frame_bgr):
        # BGR→RGB once, write into shared frame memory
        ...
        # Drain remaining tiles as workers finish
    ...
    def shutdown(self):
        ...
        for e in self.tile_ready:
            e.set()   # unblock any worker waiting on tile_ready
        for w in self.workers:
            w.join(timeout=3)
...
if __name__ == "__main__":
    mp.set_start_method("spawn")   # required on Jetson / CUDA processes
    ...
    try:
        while True:
            ...
            for loc in locations:
                ...
                print(
                    f"{loc.label}: {loc.conf:.1f}%  depth={depth_str}  "
