...
class NetworkPool:
    """
    Maintains a fixed number of darknet network handles.
    Each worker thread acquires its own handle, runs inference,
    then releases it — no locking needed inside detect_image.
    """
...
def darknet_infer(tile_bgr):
    """Acquire a private network handle, run inference, release it."""
...
class ObjectDetector:
    def find_objects(self, frame_bgr):
        """
        Submits all tiles to the thread pool concurrently, each using its own
        network handle from the pool. Returns [(label, conf, (x1,y1,x2,y2)), ...]
        sorted by area descending.
        """
        tiles = get_tiles(frame_bgr, NET_W, NET_H, OVERLAP)

        # Submit all tiles concurrently — each call blocks only on its own handle
        futures = {
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # Rolling average loop time in the window title
            if frame_count % 30 == 0:
                avg_ms = total_ms / frame_count
                print(f"[perf] avg loop: {avg_ms:.1f}ms  ({1000/avg_ms:.1f} fps)")

            cv2.putText(rgb, f"{elapsed_ms:.0f}ms", (DISPLAY_W - 80, 24),
            ...
            elif elapsed_ms > TARGET_LOOP_MS + 5:
                print(f"Loop overrun: {elapsed_ms - TARGET_LOOP_MS:.1f}ms")
        ...
        # Free all network handles in the pool
